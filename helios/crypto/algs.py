...
class EGCiphertext:
    ...
    def generate_disjunctive_encryption_proof(self, plaintexts, real_index, randomness, challenge_generator):
        # note how the interface is as such so that the result does not reveal
        # which is the real proof.

        proofs = [None for p in plaintexts]

        # go through all plaintexts and simulate the ones that must be
        # simulated.
        for p_num in range(len(plaintexts)):
            if p_num != real_index:
                proofs[p_num] = self.simulate_encryption_proof(
                    plaintexts[p_num])
        ...
        def real_challenge_generator(commitment):
            ...
            # get the commitments in a list and generate the whole disjunctive
            # challenge
            commitments = [p.commitment for p in proofs]
            disjunctive_challenge = challenge_generator(commitments)

            # now we must subtract all of the other challenges from this
            # challenge.
    ...
    def verify_disjunctive_encryption_proof(self, plaintexts, proof, challenge_generator):
        ...
        if len(plaintexts) != len(proof.proofs):
            print("bad number of proofs (expected %s, found %s)" %
...
class DLogProof(object):
    ...
    def from_dict(cls, d):
        dlp = cls(
            int(d['commitment']), int(d['challenge']), int(d['response']))
        return dlp
